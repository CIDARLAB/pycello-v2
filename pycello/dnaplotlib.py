            ...
            for i, component in enumerate(group.components):
                for j, part_instance in enumerate(component.parts):
                    ...
                    if part_instance.part.type == 'promoter':
                        upstream = netlist_utils.get_upstream_node(part_instance.part, component.node, netlist)
                        color = 'black'
                        if (upstream):
                            upstream_components = netlist_utils.get_components(upstream, placement)
                            if (len(upstream_components)):
                                cds = netlist_utils.get_cds(upstream_components[0])
                                color = cds.color
                        part = {'type': 'Promoter',
                                'fwd': True,
                                'start': start,
